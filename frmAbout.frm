...
Private Enum SomeConsts
    ...
    toClose = 1
    Neither = 0
    SnipOff = 10 'off thr button corners
    SND_ASYNC = 1 'stop other sounds
    SND_NODEFAULT = 2 'there's no default sound
    SND_APPLICATION = &H80 'it's a system sound
    SoundFlags = SND_ASYNC Or SND_NODEFAULT Or SND_APPLICATION
End Enum
#If False Then
...
Private Sub tmrSlide_Timer()
    ...
    Select Case CntDelay
      Case 40
        OpenClose = toOpen
        PlaySound "SystemStart", 0&, SoundFlags
      Case 41
        CntDelay = 40
    End Select
    Select Case Rounds
      Case MaxRounds
        ...
        If OpenClose <> toClose Then
            PlaySound "SystemExit", 0&, SoundFlags
            OpenClose = toClose
        End If
      Case Is > MaxRounds
...
':) Ulli's VB Code Formatter V2.21.6 (2006-Aug-31 15:20)  Decl: 78  Code: 336  Total: 414 Lines
':) CommentOnly: 23 (5,6%)  Commented: 26 (6,3%)  Empty: 96 (23,2%)  Max Logic Depth: 4
