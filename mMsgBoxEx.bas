  ...
  Dim TmpPosn       As typPT
    ...
    If nCode = HCBT_ACTIVATE Then
        If Left$(ClassName, GetClassName(wParam, ClassName, Len(ClassName))) = MsgBoxClassName Then 'called by message box just opening
            hWndBox = wParam 'save hWnd for timer callback, so we don't have to find it again 'NOT MYOPACITY...
...
Public Function MsgBoxEx(ByVal Prompt As String, _
  ...
  Dim TimerId       As Long
    ...
    End If
    IsYesNoBox = ((Buttons And 15) = vbYesNo) 'yes/no boxes are special

    MsgBoxEx = MsgBox(Prompt, Buttons, CurrMsgBoxTitle) 'display the box
...
Private Sub TimerCallback(hWnd As Long, uMsg As Long, idEvent As Long, dwTime As Long)
  ...
  'close timed message box
    ...
    If IsYesNoBox Then 'vbYesNo-message boxes do not react to WM_CLOSE so we try and send it the shortcut char for 'No' instead of sending WM_CLOSE
        SendKeys Left$(Replace$(Substitute(Nein), "&", vbNullString), 1) 'try sending whatever char there is for "No"
      Else 'ISYESNOBOX = FALSE/0
        SendMessage hWndBox, WM_CLOSE, Nil, ByVal Nil
    End If
    hWndBox = Zero

End Sub

':) Ulli's VB Code Formatter V2.21.6 (2006-Aug-31 15:20)  Decl: 148  Code: 172  Total: 320 Lines
':) CommentOnly: 75 (23,4%)  Commented: 52 (16,2%)  Empty: 39 (12,2%)  Max Logic Depth: 7
